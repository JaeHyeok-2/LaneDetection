...
from threshold import gradient_combines, hls_combine, comb_result
...
if __name__ == "__main__":
    ...
    if input_type == "image":
        ...
        combined_gradient = gradient_combines(undist_img, th_sobelx, th_sobely, th_mag, th_dir) 
        combined_hls = hls_combine(undist_img, th_h, th_l, th_s) 
        combined_result = comb_result(combined_gradient, combined_hls)

        c_rows, c_cols = combined_result.shape[:2]  # 540 ,720 , 1의 이미지일 텐데, 
        s_LTop2, s_RTop2 = [c_cols / 2 - 24, 5], [c_cols / 2 + 24, 5] # 이미지를 반으로 나누었을 때 아랫부분에서 왼쪽 상단, 
        s_LBot2, s_RBot2 = [110, c_rows], [c_cols - 110, c_rows] # 이건 하늘 쪽 이미지 아닌가
        ...
        while (cap.isOpened()):
            ...
            info2[5:110, cols-110:cols-6] = (255 ,255 ,255) 

                
            info = cv2.addWeighted(result, 1, info, 0.2, 0) 
            info2 = cv2.addWeighted(result, 1, info2, 0.2 ,0)
